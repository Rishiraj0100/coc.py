...
class COCHTML5Translator(HTML5Translator):
...
class COCStandaloneHTMLBuilder(StandaloneHTMLBuilder):
...
def add_builders(app):
    ...
    app.set_translator('html', COCHTML5Translator, override=True)
    app.add_builder(COCStandaloneHTMLBuilder, override=True)
    ...
    try:
        ...
        injected_mro = tuple(base if base is not StandaloneHTMLBuilder else COCStandaloneHTMLBuilder
                             for base in original.mro()[1:])
        new_builder = type(original.__name__, injected_mro, {'name': 'readthedocs'})
        app.set_translator('readthedocs', COCHTML5Translator, override=True)
